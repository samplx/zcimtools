...
C            THIS CAN ALSO BE CAUSED BY INSUFFICENT SYMBOL SPACE WHEN
C            AN INLINE CONSTANT IS BEING DEFINED (SEE CARD 44130).
C
...
C           ERROR.
C
C   153     ILLEGAL CONTROL RECORD:  FIRST NONBLANK CHARACTER WAS
C           OTHER THAN A DOLLAR SIGN.
C
C   154     BAD CODE ORIGIN FROM PASS-1 (E.G. PASS-1 ERROR 48).
C
C
...
C
C               THE SIZE OF MEM USED TO BE 2500.
C
C
...
C      4     MAG     16          4     MAG       17
C      5     DEC     9           5     DEC       10
C      6     DISK    22          6     DISK      20
C      7     DISK    23          7     DISK      21
C
C
...
C
C     GLOBAL VARIABLES
      INTEGER SYMBOL(6500),SYMAX,SYTOP,SYINFO,LMEM
     ...
     1    ITRAN,OTRAN
      ...
      INTEGER MAXMEM,MEMTOP,MEMBOT,MEM(6144),MAXVM,OFFSET,PREAMB
...
C
     ...
     5   CY,ACC,CARRY,ZERO,SIGN,PARITY
      ...
      INTEGER GNC
      common /ercnt0/ercnt0
      integer ercnt0
C     Modification to remove fixed channels - Mark Ogden
      integer ofile
C     INITIALIZE MEMORY
      CALL INITAL
...
2     CONTRL(I) = -1
      ...
      CONTRL(16) = 0
      CONTRL(17) = 1
C      *** ABOVE CHANGED FROM ORIGINAL CONTRL(17)=0
      CONTRL(18) = 1
C      *** ABOVE CHANGED FROM ORIGINAL CONTRL(18) = 0
      CONTRL(19) = 0
      CONTRL(20) = 1
      CONTRL(21) = 4
      CONTRL(23) = 1
      ...
      CONTRL(25) = 0
      CONTRL(26) = 2
C      *** CHANGED FROM ORIGINAL CONTRL(26)=1 ***
      CONTRL(27) = 0
      CONTRL(28) = 1
      CONTRL(29) = 73
      CONTRL(30) = 0
      CONTRL(31) = 1
C      *** ABOVE CHANGED FROM ORIGINAL CONTRL(31)=1 ***
      CONTRL(32) = 7
      CONTRL(33) = 0
      CONTRL(34) = 120
C      *** CHANGED FROM ORIGINAL CONTRL(34) = 72
      CONTRL(36) = 1
      CONTRL(37) = 2
      CONTRL(47) = 0
C
...
C         DUMP THE PREAMBLE
          I = OFFSET
          OFFSET = 0
      IF (INTPRO(1) .EQ. 0) GO TO 12
      CALL DUMP (0,2,16,1)
12    IBEGIN = 8
      IEND = 10
      DO 14 II = 2,8
      IF (INTPRO(II) .EQ. 0)GO TO 13
      CALL DUMP (IBEGIN,IEND,16,1)
13    IBEGIN = IBEGIN + 8
      IEND = IEND + 8
14    CONTINUE
          OFFSET = I
...
90    ISAVE = CODLOC
      CALL LOADIN (KVAL)
      IF (CODLOC .EQ. ISAVE) GO TO 100
C     DUMP THE INITIALIZED VARIABLES
      IF (CONTRL (17) .NE. 0) CALL DUMP (KVAL,CODLOC-1,16,16)
...
C     PUNCH DECK
      ...
      IBEGIN = 8
      IEND = 10
      IF (INTPRO (1) .EQ. 0) GO TO 200
      CALL PUNCOD (0,2,MODE1)
      MODE1 = 3
200   CONTINUE
      DO 220 II = 2, 8
      IF (INTPRO(II) .EQ. 0) GO TO 210
      CALL PUNCOD (IBEGIN,IEND,MODE1)
      MODE1 = 3
210   IBEGIN = IBEGIN + 8
      IEND = IEND + 8
220   CONTINUE
          OFFSET = K
      IF (CODLOC .EQ. ISAVE ) GO TO 230
      CALL PUNCOD (OFFSET+PREAMB,ISAVE-1,MODE1)
      CALL PUNCOD(KVAL,CODLOC-1,3)
      GO TO 240
230   CALL PUNCOD(OFFSET+PREAMB,CODLOC-1,MODE1)
240   CONTINUE
      IDUMY = 0
      CALL PUNCOD ( IDUMY,IDUMY,2)
          CALL WRITEL(0)
          CONTRL(26) = I
C
9999  CONTINUE
...
10100 CONTINUE
C
C    PRINT OUT USAGE STATISTICS FOR PLM2.
C
      CALL CMPUSE
      
C     tell the (0,0) overlay about the errors
...
C     modified back to V2.0 code for portability
      J = MEM(J)
      K = MOD(I,WFACT)+1
...
C     changes to revert to V2.0 channel usage - Mark Ogden
      READ(IFILE, 1000, end=80) IBUFF
      GO TO 100
C     here if end-of-file encountered.
C
C     set contrl(16) to one to flag end-of-file, and jump to 999.
      
80    contrl(16) = 0
      go to 999
      
C     here if not end-of-file
...
C     revert to V2.0 channel usage
      OFILE = CONTRL(26) + 10
9     CONTINUE
...
C         PERFORM ACC-IMMEDIATE OPERATION
          CALL EMIT(OP,negate(MOD(K,256)),0)
          GO TO 700
C
...
9999  RETURN
      ...
      integer function negate(number)
      
CCC  negate  -  return the negative of a number
C
C    negate is used rather than a simple unary minus to avoid
C    the introduction of negative zero's.
      
      negate = - number
      if(number.eq.0)negate = 0
      return
      end
...
C     ARRIVE HERE WITH PROC, VARB, OR LABEL
545   IF (I.EQ.VARB) J = 1
      IF (I.EQ.PROC) J = 3
      IF (I.EQ.LABEL) J = J + 1
      DO 555 I = 1, J
          SYMBOL(SYINFO) = 0
...
C     MAY BE POSSIBLE TO LXI
      IF (IB.NE.(IA-1)) GO TO 2010
          CALL EMIT(LXI,IB,L)
          GO TO 2210
2010      CALL EMIT(LD,IA,negate(LP))
          GO TO 2200
C
...
2150      IF (LP.EQ.0) CALL EMIT(XR,RA,0)
          IF (LP.NE.0) CALL EMIT(LD,RA,negate(LP))
C
2200  IF (IB.EQ.0) GO TO 2300
          CALL EMIT(LD,IB,negate(L/256))
2210  REGS(IB) = S
          REGV(IB) = -L
C
...
C     GENERATE LOW ORDER BYTE STORE
      IF (I2.EQ.0) GO TO 200
          ...
          GO TO 300
C     IMMEDIATE STORE
200       CALL EMIT(LD,ME,negate((MOD(IABS(L),256))))
300   CONTINUE
...
C     STORE HIGH ORDER ZERO
      IF((I2 .NE. 1) .OR. (KEEP .EQ. 0)) GO TO 1101
...
C         LRI OPERATION
          IF (LASTLI .NE. CODLOC-2) GO TO 1100
          IF (LASTRI/2 .NE. OPA/2)  GO TO 1100
          IF ((LASTRI .GE. 8) .OR. (LASTRI .LE. 1)) GO TO 1100
C       HERE WE HAVE A LRI OPERATION FOLLOWING A LRI TO THE OTHER
C       REGISTER IN THIS PAIR (B/C,D/E,H/L) OR TO THE SAME REG.
          IF (LASTRI .NE. OPA) GO TO 1010
C       HERE TO SAME REGISTER
          CALL PUT(CODLOC-1,negate(opb))
          GO TO 11000
C
C       HERE TO THE OTHER REGISTER IN THE PAIR
C         WE HAVE TO CHANGE THE OPCODE TO A LXI OF THE HIGH REG (B,D,H)
C         ALSO WE WILL ONLY ADD 1 WORD AND WE CAN CLEAR LASTLI
1010      BITS(1) = (GET(CODLOC-2) / 16 * 16) + 1
          CALL PUT (CODLOC-2,BITS(1))
          N = 1
          LASTLI = 0
          IF (MOD(OPA,2) .EQ. 1) GO TO 1020
C       HERE THE LOW REGISTER (C,E,L) WAS ALREADY THERE SO WE JUST ADD
C       THE HIGH REG.
          BITS(1) = negate(OPB)
          GO TO 10000
C
C       HERE THE HIGH REGISTER (B,D,H) WAS ALREADY THERE SO WE NEED TO
C       INSERT THE LOW REGISTER BETWEEN THE OPCODE AND IT.
1020      BITS(1) = GET(CODLOC-1)
          CALL PUT(CODLOC-1,negate(opb))
          GO TO 10000
C
C       HERE THE LRI DIDN-T FOLLOW A LRI THAT WE COULD CHANGE
1100      LASTLI = CODLOC
          LASTRI = OPA
          N = 2
...
C         IMMEDIATE OPERAND
          IF (OPA .NE. 0) GO TO 2100
          GO TO (2010,2100,2010,2100,2020,2010,2010,2100), OPR-3
C                 AD   AC   SU   SB   ND   XR   OR   CP
C
C  HERE WE HAVE A ADI,0 SUI,0 XRI,0 OR ORI,0 WE JUST TURN IT
C  INTO A ORA,A
2010      BITS(1) = CBITS(OR) + 7
          GO TO 10000
C
C  HERE WE HAVE A ANI,0 WE JUST TURN IT INTO A XRA,A
2020      BITS(1) = CBITS(XR) + 7
          GO TO 10000
C
C  HERE WE CAN NOT OPTIMIZE THE IMMEDIATE OPERAND
C
2100      N = 2
      BITS(1) = BITS(1) + 70
          BITS(2) = negate(OPA)
...
C
      TEMP1 = OFFSET + PREAMB
      IF (OFFSET .NE. 0) GO TO 505
      IF (CONTRL(47) .EQ. 1) GO TO 505
      TEMP1 = 0
505   CALL CONOUT (1,4,TEMP1,16)
      OBP = OBP - 1
      CALL PAD (1,2,1)
      CALL PAD (1,3,1)
      ISUM = 1 + RIGHT (TEMP1,8) + SHR (TEMP1,8)
      ISUM = RIGHT (ISUM,8)
      ISUM = MOD (256-ISUM,256)
      CALL CONOUT (1,2,ISUM,16)
      OBP = OBP - 1
...
710       CONTINUE
      IF (INTPRO(1) .NE.0) GO TO 715
      IF (OFFSET .NE. 0) GO TO 715
      IF (CONTRL(47) .EQ. 1) GO TO 715
...
C
2000  CONTINUE
C     TYP: 0=OPR, 1=ADR, 2=VAL, 3=DEF, 4=LIT, 5=LIN.
      TYP = TYP+1
      SP = SP + 1
...
C     CHECK FOR PREVIOUS REFERENCE  FORWARD
      IF ((I .EQ. 0) .OR. (I.EQ.-1)) GO TO 6010
      L = MOD(I,256)
      I = I/256
      J = MOD(I,512)
      I = I/512
...
C        NOTE IN A DO CASE BLOCK, AN IMPLICIT GOTO IS
C        GENERATED AT THE END OF EACH SELECTIVE STATEMENT
C        IN ORDER TO BRANCH OUT TO THE NEXT STATEMENT
C        AT THE END OF BLOCK. ALL THESE GOTOS ARE
C        CONSIDERED A SINGLE FORWARD REFERENCE FOR THE
C        THE CONVIENANCE OF THE H/L OPTIMIZATION
C        ************************************************
C        MAKE SURE IF SAVED H/L IS ALREADY MARKED NO GOOD
      LSYM = SYMBOL (I - 3)
      IF (LSYM .EQ. -1) GO TO 20300
      KTOTAL = 0
      NFLAGH = 0
      NFLAGL = 0
      NHV = REGV(6)
...
C        /H VALID /L VALID /H VALUE /L VALUE /
      IF ((NLV .LT. 0) .OR. (NLV .GT. 255)) GO TO 20210
      NFLAGL = 1
      KTOTAL = NLV + 131072
20210 IF ((NHV .LT. 0) .OR. (NHV .GT. 511))GO TO 20215
      NFLAGH = 1
      KTOTAL = ( NHV + 1024) * 256 + KTOTAL
      GO TO 20220
20215 IF (KTOTAL .NE. 0) GO TO 20220
      SYMBOL (I-3) = -1
      GO TO 20300
20220 IF (LSYM .NE. 0) GO TO 20222
      SYMBOL(I-3) = KTOTAL
      GO TO 20300
20222 NSLV = MOD (LSYM,256)
      LSYM = LSYM / 256
      NSHV = MOD (LSYM,512)
      LSYM = LSYM / 512
      KOFLAL = MOD (LSYM,2)
      ...
      HPOK = 0
      SYMBOL (I-3) = -1
      IF ((KOFLAH .EQ. 0) .OR. (NFLAGH .EQ. 0)) GO TO 20224
      IF (NHV .EQ. NSHV) GO TO 20226
20224 HPOK = 1
      KTOTAL = KTOTAL - 262144
20226 IF ((KOFLAL .EQ. 0) .OR. (NFLAGL .EQ. 0)) GO TO 20227
      IF (NLV .EQ. NSLV ) GO TO 20228
20227 IF (HPOK .EQ. 1) GO TO 20300
      KTOTAL = KTOTAL - 131072
20228 SYMBOL (I-3) = KTOTAL
      GO TO 20300
C
...
C *** CLEAR PENDING STORE IF PASSING ADDRESS VAR TO BYTE VAR ***
      IF(REGS(RA) .EQ. JP)  REGS(RA)=0
C ***         ***    ***         ***          ***
              REGS(JP) = 0
              LOCK(JP) = 0
              JP = 0
...
22560     ID = ID - 1
          ...
          IF (REGS(ID).EQ.0) GO TO 22570
              ...
              IF (ML.EQ.ID) ML = IT
              IF (MH.EQ.ID) MH = IT
C ***  CLEAR PENDING STORE WHEN REG PAIRS ARE TO BE EXCHANGED ***
             IF(REGS(1).NE.ID) GO TO 22565
                 CALL EMIT(LD,IT,RA)
                 REGS(1)=0
                 LOCK(1)=0
                 GO TO 22566
...
44000 CONTINUE
      I = LITV(SP)
      IF (I.GE.0) GO TO 44005
          CALL ERROR(154,1)
          GO TO 99990
44005 IF (CODLOC.LE.I) GO TO 44100
          CALL ERROR(141,1)
C
44100 J = CONTRL(47)
      K = 3
...
C     WRITE OCTAL ADDRESS
      ...
      I = SYMBOL(J-1)
      I = MOD(I,16)
      IF (I.EQ.PROC.OR.I.EQ.LABEL) J = J - 2
      I = IABS(SYMBOL(J))
...
C     PRSTRASNLITV
     ...
     *  51395,  51396,  51397/
      DATA WDSIZE /31/, TWO8 /256/, MAXMEM /6144/
      END
      SUBROUTINE CMPUSE
      INTEGER SYMBOL(6500),SYMAX,SYTOP,SYINFO,LMEM
      COMMON /SYMBL/SYMBOL,SYMAX,SYTOP,SYINFO,LMEM
      ...
      COMMON /MEMORY/MAXMEM,MEMTOP,MEMBOT,MEM,MAXVM,OFFSET,PREAMB
C
C    THIS SUBROUTINE WAS ADDED AT SRI.
C
C     modifications to remove fixed channels - Mark Ogden
      INTEGER CONTRL(64)
      COMMON /CNTRL/CONTRL
      INTEGER ofile
      ...
      ofile = CONTRL(26) + 10
      write(ofile,1000)
C    PRINT OUT THE SYMBOL TABLE AND MEMORY TABLE POINTERS.
      WRITE(ofile,1001) SYMAX,SYTOP,SYINFO,MAXMEM,MEMTOP,MEMBOT
      RETURN
C
1000  format ("0table usage in pass 2:"/)
1001  FORMAT (" SYMBOL TABLE - MAX=",I6," TOP=",I6," INFO=",I6,
     1       /" MEMORY TABLE - MAX=",I6," TOP=",I6,"  BOT=",I6)
      END
